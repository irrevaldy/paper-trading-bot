...
class TrendVolumeImbalanceV3Strategy(BaseStrategy):
    ...
    def evaluate(self, symbol: str, now_ts: float) -> Signal:
        ...
        if settings.enable_trailing_stop and price <= position.highest_price * (1 - settings.trailing_stop_pct):
            return Signal(symbol, "SELL", "trailing stop hit", price, 1.0)
        ...
        if orderbook["suspicious_bid_wall"] and bearish_trend:
